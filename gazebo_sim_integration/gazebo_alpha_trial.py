...
from gazebo_base_cmd_publisher import GazeboBaseCmdPublisher
...
def parse_args():
    ...
    p.add_argument("--sim-base-enabled", action="store_true",
                   help="Publish rover base Twist commands to Gazebo.")
    p.add_argument("--sim-base-cmd-topic", default="/model/vehicle_blue/cmd_vel",
                   help="Gazebo ROS topic for rover base velocity commands.")
    return p.parse_args()
...
def main():
    ...
    base_cmd_pub = None
    if actuation == "sim" and bool(getattr(args, "sim_base_enabled", False)):
        base_cmd_pub = GazeboBaseCmdPublisher(topic_name=str(args.sim_base_cmd_topic))
    ...
    try:
        while True:
            ...
            if base_cmd_pub is not None:
                try:
                    cmd_v = float(getattr(last_twist, "v_mps", 0.0))
                    cmd_w = float(getattr(last_twist, "w_rps", 0.0))
                    if bool(hard_stop):
                        cmd_v = 0.0
                        cmd_w = 0.0
                    base_cmd_pub.publish(cmd_v, cmd_w)
                except Exception as e:
                    print(f"[WARN] Gazebo base publish failed: {e}")
        ...
        try:
            ...
            pass
        try:
            if 'base_cmd_pub' in locals() and base_cmd_pub is not None:
                base_cmd_pub.stop()
                base_cmd_pub.close()
        except Exception:
            pass
