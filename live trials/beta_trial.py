...
def parse_args():
    ...
    p.add_argument("--infer-min-interval", type=float, default=0.15,
                   help="If >0, run vision inference at most once every N seconds and reuse last detections between calls (helps prevent sensor/health HALT flapping on slow models).")
    ...
    p.add_argument("--trial-halt", action="store_true", default=False,
                   help="Enable HALT transitions from health/safety policies (default: disabled for trials).")
    p.add_argument("--trial-nohalt", action="store_true", default=False,
                   help="Deprecated alias. If set, disables HALT (same as omitting --trial-halt).")
...
def main():
    ...
    trial_nohalt = (not is_ghost) and (not bool(getattr(args, "trial_halt", False)))
    ...
    # metrics
    run_tag = f"beta_trail_{sensor_mode}_{op_mode}_{time.strftime('%Y%m%d_%H%M%S')}"
    ...
    win = f"BETA_TRAIL (LIVE) [{getattr(args, 'vision_backend', 'ultralytics')}]"
    cv2.namedWindow(win, cv2.WINDOW_NORMAL)

    # -----------------------
    # Conference HUD helpers
    # -----------------------
    ...
    def _model_label_ui() -> str:
        try:
            backend = str(getattr(args, "vision_backend", "") or "").strip().lower()
        except Exception:
            backend = ""
        if backend == "roboflow":
            mid = str(getattr(args, "rf_model_id", "") or "").strip()
            return f"Roboflow:{mid}" if mid else "Roboflow"
        mp = str(getattr(args, "model_path", "") or "").strip()
        mp = os.path.expanduser(mp)
        base = os.path.basename(mp) if mp else ""
        low = base.lower()
        ...
        if low.endswith(".pt"):
            return "PT"
        return base or "YOLOv11"

    MODEL_UI = _model_label_ui()

    # inference caching (optional)
    last_inf_ts = 0.0
    last_inf = None

    # HALT hysteresis to prevent rapid flapping when sensors briefly miss deadlines
    HALT_ENTER_CONFIRM_S = 0.6
    HALT_EXIT_CONFIRM_S = 1.2
    _halt_enter_t0 = None
    _halt_exit_t0 = None
    _plan_halt_active = False
    ...
    try:
        while True:
            ...
            # vision inference (optionally throttled)
            infer_min_interval = float(getattr(args, "infer_min_interval", 0.0) or 0.0)
            if (infer_min_interval > 0.0) and (last_inf is not None) and ((now - last_inf_ts) < infer_min_interval):
                inf = last_inf
            else:
                inf = detector.infer(frame)
                last_inf = inf
                last_inf_ts = time.time()  # measure interval from inference completion
            ...
            if not is_ghost:
                ...
                if dataclasses.is_dataclass(plan):
                    plan_cam_count = expected_cam if bool(getattr(args, "strict_cam_health", False)) else 1
                    plan = dataclasses.replace(plan, camera_count=plan_cam_count, lidar_on=expected_lidar_on)
                # Debounce HALT so brief sensor hiccups do not toggle modes repeatedly
                plan_mode_str = str(getattr(plan, "mode", "") or "")
                want_halt = ("HALT" in plan_mode_str.upper())

                if want_halt:
                    _halt_exit_t0 = None
                    if not _plan_halt_active:
                        _halt_enter_t0 = _halt_enter_t0 or now
                        if (now - _halt_enter_t0) >= HALT_ENTER_CONFIRM_S:
                            _plan_halt_active = True
                else:
                    _halt_enter_t0 = None
                    if _plan_halt_active:
                        _halt_exit_t0 = _halt_exit_t0 or now
                        if (now - _halt_exit_t0) >= HALT_EXIT_CONFIRM_S:
                            _plan_halt_active = False
                    else:
                        _halt_exit_t0 = None

                if _plan_halt_active:
                    # Force HALT mode in the applied plan (best-effort)
                    try:
                        plan = dataclasses.replace(plan, mode=SystemMode.HALT)
                    except Exception:
                        pass
            ...
            # UI overlay
            # UI overlay (conference HUD + action strip)
            disp = cv2.resize(frame, (960,540))

            # Derive ACTION (SCOOP/BYPASS/STOP) + SAFETY (NORMAL/SAFE_HOLD/HALT)
            action = "BYPASS"
            if label == "harvestable":
                action = "SCOOP"
            if (label == "object_hazard") or bool(hard_stop):
                action = "STOP"

            safety_ui = "NORMAL"
            try:
                plan_mode = str(getattr(plan, "mode", "") or "")
            except Exception:
                plan_mode = ""
            if plan is not None and ("HALT" in plan_mode.upper()):
                safety_ui = "HALT"
            elif bool(hard_stop) or bool(soft_bypass) or (label in ("object_hazard","human_hazard")):
                safety_ui = "SAFE_HOLD"

            # Update action history (keep last 10)
            try:
                action_hist.append((now, action, dist_m, top_cls, top_conf))
            except Exception:
                pass
            ...
            cv2.putText(canvas, f"MODEL: {MODEL_UI}", (x0, y), font, 0.55, (255,255,255), 1); y += dy
            cv2.putText(canvas, f"TOP: {top_cls} @ {top_conf}", (x0, y), font, 0.52, (230,230,230), 1); y += dy
            cv2.putText(canvas, f"ACTION: {action}", (x0, y), font, 0.60, (255,255,255), 1); y += dy
            cv2.putText(canvas, f"SAFETY: {safety_ui}", (x0, y), font, 0.60, (255,255,255), 1); y += dy

            dist_str = "--" if dist_m is None else f"{dist_m:.2f}m"
            cv2.putText(canvas, f"LiDAR min: {dist_str}", (x0, y), font, 0.52, (210,210,210), 1); y += dy
            cv2.putText(canvas, f"NAV: {nav_mode.value}", (x0, y), font, 0.52, (210,210,210), 1); y += dy
            cv2.putText(canvas, f"CLIP: {clipper.active}:{clip_step}", (x0, y), font, 0.52, (210,210,210), 1); y += dy
            cv2.putText(canvas, f"CAMS: {cam_ok}/{expected_cam}", (x0, y), font, 0.50, (190,190,190), 1); y += dy

            if getattr(args, "debug_rf", False) and rf_dbg:
                cv2.putText(canvas, rf_dbg[:28], (x0, y), font, 0.40, (180,255,180), 1); y += int(dy*0.9)
            if getattr(args, "debug_cam", False) and pkt_dbg:
                cv2.putText(canvas, pkt_dbg[:28], (x0, y), font, 0.40, (180,180,255), 1); y += int(dy*0.9)

            y += 10
            cv2.putText(canvas, "Recent (last 10):", (x0, y), font, 0.50, (220,220,220), 1); y += dy

            rows = list(action_hist)[-10:]
            for (ts, act, d, cls, conf) in rows:
                ts_s = _fmt_ts(ts)
                d_s = "--" if d is None else f"{float(d):.2f}m"
                cls_s = (str(cls) if cls else "")[:14]
                conf_s = (str(conf) if conf else "")[:5]
                line = f"{ts_s} {act:<5} {d_s:<6} {cls_s}@{conf_s}"
                cv2.putText(canvas, line, (x0, y), font, 0.42, (200,200,200), 1)
                y += 18

            # Minimal HUD on video (non-CS friendly)
            cv2.rectangle(canvas, (0,0), (960,60), (0,0,0), -1)
            cv2.putText(canvas, f"DECISION={label}  ACTION={action}  SAFETY={safety_ui}", (10,25),
                        font, 0.6, (255,255,255), 1)
            cv2.putText(canvas, f"TOP={top_cls}@{top_conf}  LiDAR={dist_str}", (10,50),
            ...
            if k == ord('q'):
                break            # loop pacing (compute-aware): don't add extra delay if inference already took longer than 1/fps
            fps = max(5, min(int(args.max_fps), 60))
            loop_dt = max(0.0, time.time() - now)
            sleep_s = max(0.0, (1.0 / fps) - loop_dt)
            if sleep_s > 0:
                time.sleep(sleep_s)
    finally:
        try:
